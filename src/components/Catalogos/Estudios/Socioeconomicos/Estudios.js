...
export default function Estudios() {
  ...
  const allEstudiosFiltrados = allEstudios.filter(
    (item) =>
  ...
  const postDataEditarColaborador = () => {
    axios
      .post(`${APIURL}/estudio/preasignacion`, fromAsignarColaborador, config)
      .then((resp) => {
  ...
  const postDataAsignarColaboradores = () => {
    axios
      .post(`${APIURL}/estudio/asignar`, fromAsignarColaborador, config)
      .then((resp) => {
        console.log(resp.data);
        setSelectedRows([]);
        setSelectedOption(null);
  ...
  useEffect(() => {
    ...
    fromData.id_proyecto,
    fromData.id_cliente,
    fromData.id_orden_servicio,
    fromData.id_colaborador,
  ]);
  ...
  const mostrarContactoPrincipal = (contacto_principal) => {
    ...
    return (
      <div>
        ...
        <div>
          <b>correo:</b> {contacto_principal.email}
        </div>
        <div>
          <b>telefono:</b> {contacto_principal.telefono_casa}
        </div>
      </div>
    );
  };
  ...
  const renderFilasTablaEstudiosSocioeconomicos = () => {
    return allEstudiosFiltrados.map((estudio, index) => (
      <tr key={"tr-cliente-" + index}>
        ...
        <td>{mostrarEstado(estudio.estado)}</td>
        <td>{contactoPrincipalDireccion(estudio.contacto_principal)}</td>
        <td>{estudio.proyecto?.nombre && estudio.proyecto.nombre}</td>
        <td>{estudio.cliente?.nombre && estudio.cliente.nombre}</td>
        <td>
        ...
        return (
          <>
            <div className="row d-flex align-items-center">
              <div className="col-2 col-md-1 pe-0 mb-2">
                <label htmlFor="search" className="mb-0">
                  Buscar:
                </label>
              </div>
              <div className="col-6 col-md-3 ps-0 mb-2">
                <input
                  type="text"
          ...
          <TabPanel>
            ...
            <div style={{ overflowX: "auto", height: '50vh' }}>
              ...
              >
                ...
                <tbody>{renderFilasTablaEstudiosSocioeconomicos(1)}</tbody>
              </table>
            </div>
          </TabPanel>
          <TabPanel>
