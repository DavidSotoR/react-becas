...
function AltaEstudioLaboral() {
  ...
  const sendDataEstudioSocioeconomico = () => {
    axios
      .post(`${APIURL}/estudio/laboral`, fromData, config)
      .then((resp) => {
        console.log(resp);
      })
      .catch((resp) => {
  ...
  const getClientes = () => {
    axios
      .get(`${APIURL}/clientes?id_tipo_cliente=2`, config)
      .then((resp) => {
  ...
  const getOrdenesServicio = () => {
    axios
      .get(`${APIURL}/clientes/${cliente}/ordenes-servicio`, config)
      .then((resp) => {
  ...
  const validateFields = () => {
    ...
    if (fromData.nombre.length <= 3 || fromData.nombre === "") {
      messageError +=
        "Campo Nombre es OBLIGATORIO y debe contener mas de 3 caracteres\n";
    }
        ...
        <div className="row">
          ...
          <div className="col-md-8">
            <div className="mb-3 row">
              <label htmlFor="id_cliente" className="col-sm-2 col-form-label">
                Cliente
              </label>
            ...
            <div className="d-flex" style={{ flexDirection: "row-reverse" }}>
              <button
                ...
                onClick={sendDataEstudioSocioeconomico}
              >
                Guardar
              </button>
            </div>
