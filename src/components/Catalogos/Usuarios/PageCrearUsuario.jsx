...
export default function PageCrearUsuario() {
  ...
  const popoverGenerarContraseña = (
    <Popover id="popover-basic">
      <Popover.Body>Generar Contraseña</Popover.Body>
    </Popover>
  );
  ...
  const getDireccionGSP = () => {
    ...
    axios
      .get(
        `https://nominatim.openstreetmap.org/search?q=${dataPostUsuario.direccion}&format=json&addressdetails=1`,
        config
      )
      .then((resp) => {
  ...
  const renderFiltroClientes = () => {
    return [
      <option key="cliente-0" value="0" selected>
        Seleccione uno
      </option>,
      ,
  ...
  const validateDataFormBtn = (obj) => {
    ...
    for (let key in obj) {
      ...
      if (
        (key === "longitud" && !esExterno) ||
        (key === "latitud" && !esExterno) ||
        (key === "longitud" && esExterno) ||
        (key === "latitud" && esExterno) ||
        key === "calle" ||
        key === "numero_exterior" ||
        ...
        key === "codigo_postal" ||
        key === "pais" ||
        (key === "id_cliente" && !esExterno) ||
        key === "direccion" /* || (obj[key] !== "" && obj[key] !== "0") */
      ) {
  ...
  const validateForm = (dataForm) => {
    ...
    if (inputChanged === "id_cliente") {
      if (dataForm.id_cliente === "0") {
        ...
        errorObject.msg =
          "El campo CLIENTE es obligatorio. Seleccione una opcion.";
        if (!contieneError(errorObject.idError)) {
          setErrorsArray([...errorsArray, errorObject]);
        }
      } else {
    ...
    if (inputChanged === "name") {
      if (dataForm.name === "" || validateContieneEspacios(dataForm.name)) {
        ...
        errorObject.msg =
          "El campo NOMBRE es obligatorio. No debe contener espaciones vacios seguidos.";
        if (!contieneError(errorObject.idError)) {
          setErrorsArray([...errorsArray, errorObject]);
        }
      } else {
    ...
    if (inputChanged === "email") {
      if (!validateCorreo(dataForm.email)) {
        ...
        errorObject.msg =
          "El campo EMAIL es obligatorio. Debe ingresar un correo valido.";
        if (!contieneError(errorObject.idError)) {
          setErrorsArray([...errorsArray, errorObject]);
        }
      } else {
    ...
    if (inputChanged === "password") {
      ...
      if (!validatePassword(dataForm.password)) {
        ...
        errorObjectP.msg =
          "El campo PASSWORD es obligatorio. No debe contener espacios. Minimo 12 caracteres.";
        if (!contieneError(5)) {
  ...
  const cricleColaboradores = () => {
    ...
    const colaboradoresFiltro = colaboradores.filter(
      (colaborador) => colaborador.latitud
    );
    //console.log(colaboradoresFiltro);
    return (
      <>
        {colaboradoresFiltro.map((colaborador, index) => (
          <Circle
            key={"cum-" + index}
            center={[colaborador.latitud, colaborador.longitud]}
            radius="200"
            pathOptions={{ color: "blue" }}
          >
            <Popup>{colaborador.name}</Popup>
          </Circle>
        ))}{" "}
      </>
    );
  ...
  useEffect(() => {
    ...
    console.log(
      errorsArray.length === 0 && validateDataFormBtn(dataPostUsuario)
    );
      ...
      <div className="row mb-3">
        <div className="col-12 mb-3">
          <Form.Check type="switch" className="mx-2">
            ...
            />
            <Form.Check.Label>
              <span className="fw-bold fs-6 ms-2"> Es externo </span>
            </Form.Check.Label>
          </Form.Check>
        </div>

        <div className="col-5 mb-3">
          <label htmlFor="inputPerfil" className="form-label">
            Perfil
          </label>
          <select
            id="inputPerfil"
          ...
          {contieneError(1) && (
            <div className="text-danger fw-medium">{getErrorMsg(1)}</div>
          )}
        </div>
        <div
          className="col-5 mb-3"
          style={esExterno ? { display: "block" } : { display: "none" }}
        >
          ...
          {contieneError(2) && (
            <div className="text-danger fw-medium">{getErrorMsg(2)}</div>
          )}
        </div>
        <div className="col-5 mb-3">
          ...
          />
          {contieneError(3) && (
            <div className="text-danger fw-medium">{getErrorMsg(3)}</div>
          )}
        </div>
        ...
        <div className="col-5 mb-3">
          ...
          />
          {contieneError(4) && (
            <div className="text-danger fw-medium">{getErrorMsg(4)}</div>
          )}
        </div>
        ...
        <div className="col-12 row mb-5">
          <div className="col-12 col-md-5 col-sm-7 mb-3">
            <div className="row d-flex justify-content-start align-items-center m-0">
              <div className="col m-0 ps-0">
                ...
                <input
                  ...
                  name="password"
                  value={dataPostUsuario.password}
                  onChange={handleInputChange}
                />
              </div>
              <div className="col-1 m-0 ps-0 mt-4 pt-1">
                <OverlayTrigger
                  trigger={"hover"}
                  overlay={popoverGenerarContraseña}
                >
                  <button
                    className="btn btn-primary btn-sm btn-icon-fix btn-icon-style"
                    onClick={asignarContraseñaAutomatico}
                  >
            ...
            {contieneError(5) && (
              <div className="text-danger fw-medium">{getErrorMsg(5)}</div>
            )}
            {contieneError(0) && (
              <div className="text-danger fw-medium">{getErrorMsg(0)}</div>
            )}
          </div>
          <div className="col-12 col-md-5 col-sm-7 mb-3">
            <div className="row d-flex justify-content-start align-items-center m-0">
              <div className="col m-0 ps-0">
                <label htmlFor="inputPasswordConfirmar" className="form-label">
                  Confirmar Password
                </label>
                <input
                  type={showPassword ? "text" : "password"}
            ...
            {contieneError(6) && (
              <div className="text-danger fw-medium">{getErrorMsg(6)}</div>
            )}
            {contieneError(0) && (
              <div className="text-danger fw-medium">{getErrorMsg(0)}</div>
            )}
          ...
          <div className="row">
            <div className="col-sm-12">
              <div className="mb-3 row">
                <label htmlFor="calle" className="col-sm-2 col-form-label">
                  Calle:
                </label>
            ...
            <div className="col-sm-6">
              <div className="mb-3 row">
                ...
                <div className="col-sm-8">
                  <input
                    key={"AES-numero_exterior"}
                    type="text"
                    className="form-control"
                    id="numero_exterior"
                    name="numero_exterior"
                    value={dataPostUsuario.numero_exterior}
                    onChange={(e) => handleInputChange(e)}
                  />
                </div>
              </div>
            ...
            <div className="col-sm-6">
              <div className="mb-3 row">
                <label htmlFor="colonia" className="col-sm-4 col-form-label">
                  Colonia:
                </label>
                <div className="col-sm-8">
                  <input
            ...
            <div className="col-sm-6">
              <div className="mb-3 row">
                <label htmlFor="municipio" className="col-sm-4 col-form-label">
                  Municipio:
                </label>
                <div className="col-sm-8">
                  <input
                    ...
                    id="municipio"
                    name="municipio"
                    value={dataPostUsuario.municipio}
                    onChange={(e) => handleInputChange(e)}
                  />
                </div>
              </div>
            ...
            <div className="col-sm-6">
              <div className="mb-3 row">
                <label htmlFor="estado" className="col-sm-4 col-form-label">
                  Estado:
                </label>
                <div className="col-sm-8">
                  <input
            ...
            <div className="col-sm-6">
              <div className="mb-3 row">
                ...
                <div className="col-sm-8">
                  <input
                    key={"AES-codigo_postal"}
                    type="text"
                    className="form-control"
                    id="codigo_postal"
                    name="codigo_postal"
                    value={dataPostUsuario.codigo_postal}
                    onChange={(e) => handleInputChange(e)}
                  />
                </div>
              </div>
            ...
            <div className="col-sm-6">
              <div className="mb-3 row">
                <label htmlFor="pais" className="col-sm-4 col-form-label">
                  País:
                </label>
                <div className="col-sm-8">
                  <input
        ...
        <div
          className="col-12"
          style={{ display: !esExterno ? "block" : "none" }}
        >
            ...
            <div className="row mb-3">
              <div className="col-sm-6">
                ...
                <input
                  ...
                  id="direccion"
                  name="direccion"
                  value={dataPostUsuario.direccion}
          ...
          {cuentaConUbicacion && (
            <div className="row">
              ...
              <div className="col-12">
                <div className="mb-3 row">{seccionUbicaciones()}</div>
                <MapContainer
                  center={[dataPostUsuario.latitud, dataPostUsuario.longitud]}
                  zoom={13}
                ...
                >
                  ...
                  <Marker
                    position={[
                      dataPostUsuario.latitud,
                      dataPostUsuario.longitud,
                    ]}
                  >
                    <Popup>
